...
def vehicle_share(request, user_id, vehicle_id):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            form.fields['cost'] = fm.IntegerField(widget=forms.TextInput(attrs={'type':'number', 'min':"10", 'required':True,'placeholder':'In Rupees (RS) ','class':'form-control'}))
...
def request_approve(request, request_id):
    ...
    passenger=VehicleSharing.objects.get(id=pass_requests.ride_id)
